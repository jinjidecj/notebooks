...
当Eden区再次满了后，把对象放入S1当中，age设置为1。然后判断S0中的对象是否需要被回收，不用回收的放到S1中，age增1。这个时候S0清空，S0被称为To区。S1变成From区。依此循环。
...
当没有开启逃逸分析时，即：-XX:-DoEscapeAnalysis，程序打印：花费时间76ms，内存情况如下：
